...
if __name__ == "__main__":
    ...
    if args.llama:
        model = LlamaForCausalLM.from_pretrained(args.ckpt_path, torch_dtype=load_type, device_map='auto')
        model.config.use_flash_attention = True
        model.config.pad_token_id = 0
        model.config.eos_token_id = 2
    else:
        model = AutoModelForCausalLM.from_pretrained(args.ckpt_path, torch_dtype=load_type, device_map='auto')
